...
| 指标 | 口径 |
| --- | --- |
| 详情访问率 | 所选游戏/版本当日产生 `game_detail_view` 的去重 `session_id` ÷ 当日全站产生 `page_view` 的去重 `session_id` |
...
| 加载耗时 P75 | 按 `load_id` 去重后的 `game_ready.duration_ms` 连续第 75 百分位，四舍五入到整数毫秒 |
| 有效游玩率 | D 日产生 `game_start`，且在 D 与 D+1 收到的心跳累计 `active_seconds ≥ 300` 的 `play_id` ÷ D 日产生 `game_start` 的 `play_id` |

指标支持按日期、游戏和版本筛选。所有日归档均按服务端 `received_at` 的上海自然日计算，只统计 `metadata.environment = production`。详情访问率筛选游戏或版本时只改变分子，分母仍为当日全站首页会话；同一会话可访问多个游戏，所以不同游戏的详情访问率之和可能超过 100%。若未来需要「卡片曝光 → 详情」口径，必须先新增卡片曝光事件。

有效游玩的最终汇总日固定为上海日 D-2：每日任务只最终化当前上海日期前两天，让 D 日跨午夜继续游玩的心跳拥有完整 D+1 缓冲。汇总写入、成功状态登记处于同一事务，且成功状态最后写入；未到 D-2 或汇总失败时不会获得原始事件删除资格。
...
暂不实现 7 日回访、置信区间、A/B 测试、复杂分群、精确机器人识别、收入指标或任意 `metadata` 报表。7 日回访不会创建 `visitor_first_seen` 或看板占位；未来启用后只能从启用日起积累，原始事件超过 30 天后无法精确补算更早历史。

原始事件按服务端 `received_at` 滚动保留 30×24 小时，且只能在对应上海日的汇总成功状态写入后删除；`daily_game_metrics` 日汇总长期保存。删除任务、安全闸门和五项日汇总均已部署；首次维护已完成且未删除原始事件，后续由每日 Cron 持续最终化 D-2 和补齐漏跑日期。
